...
from math import asin, atan2, cos, fabs, floor, sin, sqrt
...
ion_default = np.array(
    [  # 2004/1/1
        ...
        [0.1167e06, -0.2294e06, -0.1311e06, 0.1049e07],
    ]
)
# troposhere model
nmf_coef = np.array(
...
def sat2id(sat):
    ...
    sys, prn = sat2prn(sat)
    gnss_tbl = {uGNSS.GPS: 'G', uGNSS.GAL: 'E', uGNSS.BDS: 'C', uGNSS.QZS: 'J', uGNSS.GLO: 'R'}
    if sys == uGNSS.QZS:
        prn -= 192
    elif sys == uGNSS.SBS:
        prn -= 100
...
def id2sat(id_):
    ...
    gnss_tbl = {'G': uGNSS.GPS, 'E': uGNSS.GAL, 'C': uGNSS.BDS, 'J': uGNSS.QZS, 'R': uGNSS.GLO}
...
def satexclude(sat, var, svh, nav):
    ...
    *-----------------------------------------------------------------"""
    ...
    if sat in nav.excsats:
        return 1
    if svh:
        trace(3, f'unhealthy satellite: sat={sat:d} svh={svh:x}\n')
        return 1
    if var > MAX_VAR_EPH:
        trace(3, f'invalid ura satellite: sat={sat:3d} ura={np.sqrt(var):.2f}\n')
        return 1
    return 0
...
def xyz2enu(pos):
    ...
    sl = sin(pos[1])
    cl = cos(pos[1])
    E = np.array([[-sl, cl, 0], [-sp * cl, -sp * sl, cp], [cp * cl, cp * sl, sp]])
    return E
...
def tropmodel(t, pos, el, humi):
    ...
    # saastamoinen model
    z = np.pi / 2.0 - el
    trop_hs = 0.0022768 * pres / (1.0 - 0.00266 * np.cos(2 * pos[0]) - 0.00028e-3 * hgt) / np.cos(z)
...
def trace(level, msg):
    if level <= trace_level:
        sys.stderr.write(f'{level:d} {msg}')
