...
from rtkcmn import rCST, sat2prn, time2epoch, timeadd, timediff, trace, uGNSS
    ...
    if sys != uGNSS.GLO:
        ...
        for i, eph_ in enumerate(nav.geph[nav.eph_index[sat] :]):
            ...
            if abs(dt) <= dt_p:
                ...
                break
    trace(4, f'seleph: sat={sat:d} dt={dt_p:.0f}\n')
...
def eph2pos(t, eph):
    ...
    *          (tgd or bgd)"""
    ...
    trace(
        4,
        f'eph2pos: sat={eph.sat:d}, dts={dts:.10f} rs={rs[0]:.4f} {rs[1]:.4f} {rs[2]:.4f} var={var:.3f}\n',
    )
    return rs, var, dts
...
def eph2clk(time, eph):
    ...
    trace(
        4,
        f'ephclk: t={t:.12f} ts={ts:.12f} dts={dts:.12f} f0={eph.f0:.12f} f1={eph.f1:.9f} f2={eph.f2:.9f}\n',
    )
    return dts
...
def geph2clk(time, geph):
    ...
    trace(4, f'geph2clk: t={t:.12f} ts={ts:.12f} taun={geph.taun:.12f} gamn={geph.gamn:.12f}\n')
...
def satposs(obs, nav):
    ...
    *          signal transmission time"""
    ...
    trace(
        3,
        f'satposs  : teph= {ep[0]:04d}/{ep[1]:02d}/{ep[2]:02d} {ep[3]:02d}:{ep[4]:02d}:{ep[5]:06.3f} n={n:d} ephopt={0:d}\n',
    )
    ...
    for i in np.argsort(obs.sat):
        ...
        if eph is None:
            svh[i] = 1
            trace(2, f'No broadcast ephemeris: sat={sat:d}\n')
            continue
        svh[i] = eph.svh
        ...
        trace(
            4,
            f'satposs: {obs.sat[i]:d},time={t.sec:.9f} dt={dt:.9f} pr={pr:.3f} '
            f'rs={rs[i, 0]:13.3f} {rs[i, 1]:13.3f} {rs[i, 2]:13.3f} '
            f'dts={dts[i] * 1e9:12.3f} var={var[i]:7.3f}\n',
        )
