...
from rtkcmn import DTTOL, rCST, sat2freq, sat2prn, timediff, trace, tracemat, uGNSS
...
def zdres_sat(nav, obs, r, rtype, dant, ix):
    ...
    for f in range(nf):
        ...
        #     trace(
        #         3,
        #         f'zdres_sat: {obs.sat[ix]:d}: L={obs.L[ix, f]:.6f} P={obs.P[ix, f]:.6f} r={r:.6f} f={freq:.0f}\n',
        #     )
    return y
...
def zdres(nav, obs, rs, dts, svh, var, rr, rtype):
    ...
    e = np.zeros((n, 3))
    rr_ = rr.copy()
    trace(3, f'zdres: n={n:d} rr={rr[0]:.2f} {rr[1]:.2f} {rr[2]:.2f}\n')
    pos = gn.ecef2pos(rr_)
    ...
    for i in ix:
        ...
        trace(
            4,
            f'sat={obs.sat[i]:d} r={r:.6f} c*dts={_c * dts[i]:.6f} zhd={zhd:.6f} map={mapfh:.6f}\n',
        )
    ...
    for i in ix:
        if obs.L[i, 0] != 0 and rtype == 0:
            trace(
                4,
                f'sat={obs.sat[i]:2d} {rs[i, 0]:13.3f} '
                f'{rs[i, 1]:13.3f} {rs[i, 2]:13.3f} {dts[i]:13.10f} {np.rad2deg(el[i]):5.1f}\n',
            )
...
def varerr(nav, sys, el, f, dt, rcvstd, snr_rover, snr_base):
    ...
    if nav.err[4] > 0:  # add SNR term
        e = fact * nav.err[4]
        var += e**2 * (
            10 ** (0.1 * max(nav.snrmax - snr_rover, 0))
...
def ddres(nav, x, P, yr, er, yu, eu, sat, el, dt, obsr):
    ...
    trace(3, f"ddres   : dt={dt:.4f} ns={ns:d}\n")
        ...
        for f in range(0, nf * 2):
            ...
            for j in idx:  # loop through sats
                ...
                if abs(v[nv]) > nav.maxinno[code] * thresadj:
                    ...
                    trace(
                        3,
                        f"outlier rejected: (sat={sat[i]:3d}-{sat[j]:3d} {'LP'[code]}{frq + 1:d} "
                        f"v={v[nv]:13.3f} x={x[ii]:13.3f} {x[jj]:13.3f} "
                        f"P={P[ii, ii]:.6f} {P[jj, jj]:.6f})\n",
                    )
                    H[ii, nv], H[jj, nv] = 0, 0
                    continue
                ...
                trace(
                    3,
                    f"sat={sat[i]:3d}-{sat[j]:3d} {'LP'[code]}{frq + 1:d} v={v[nv]:13.3f} "
                    f"R={Ri[nv]:9.6f} {Rj[nv]:9.6f} lock={nav.lock[sat[j] - 1, frq]:2d} "
                    f"x={x[jj]:13.3f} P={P[jj, jj]:.3f}\n",
                )
                nv += 1
                nb[b] += 1
            b += 1
...
def valpos(nav, v, R, thres=4.0):
    ...
    trace(3, f'valpos  : nv={len(v):d} thres={thres:.1f}\n')
    nv = len(v)
    fact = thres**2
    for i in range(nv):
        if v[i] ** 2 > fact * R[i, i]:
            trace(3, f'large residual (ix_sat={i:d}  v={v[i]:.3f} sig={np.sqrt(R[i, i]):.3f})\n')
    return True
    ...
    for m in range(uGNSS.GNSSMAX):
        ...
        for f in range(nav.nf):
            ...
            for i in range(k, k + ns):
                ...
                if (
                    nav.lock[sati - 1, f] >= 0
                    and nav.slip[sati - 1, f] & 2 == 0
                    and nav.el[sati - 1] >= nav.elmaskar
                ):
            ...
            for j in range(k, k + ns):
                ...
                if (
                    nav.lock[satj - 1, f] >= 0
                    and nav.slip[satj - 1, f] & 2 == 0
                    and nav.el[satj - 1] >= nav.elmaskar
                ):
...
def resamb_lambda(nav, sats):
    ...
    if s[0] <= 0.0 or nav.ratio >= nav.thresar:
        trace(
            3,
            f'resamb : validation OK (nb={nb:d} ratio={nav.ratio:.2f}\n s={s[1]:.2f}/{s[0]:.2f}\n',
        )
        ...
        trace(
            3,
            f'ambiguity validation failed (nb={nb:d} ratio={nav.ratio:.2f}\n s={s[1]:.2f}/{s[0]:.2f}',
        )
        nb = 0
    return nb, xa
...
def manage_amb_LAMBDA(nav, sats, stat, posvar):
    ...
    trace(3, f'posvar={posvar:.6f}\n')
    trace(3, f'prevRatios = {nav.prev_ratio1:.3f} {nav.prev_ratio2:.3f}\n')
    ...
    if nav.prev_ratio2 < nav.thresar and nav.nb_ar >= nav.mindropsats:
        ...
        if nav.excsat_ix < len(arsats):
            ...
            trace(3, f'AR: exclude sat {excsat:d}\n')
            excflag = True
            nav.excsat_ix += 1
        else:
    ...
    trace(
        3,
        f'lambda: nb={nb:d} r1= {nav.prev_ratio1:.3f} r2={nav.prev_ratio2:.3f} r={nav.ratio:.3f}\n',
    )
    if (
        nb >= 0
        and nav.prev_ratio2 >= nav.thresar
        and (
            nav.ratio < nav.thresar
        ...
        for i, f in zip(ix[0], ix[1]):
            nav.lock[i, f] = -dly
            dly += 2
            trace(3, f'remove sat {i + 1:d}:{f:d} lock={nav.lock[i, f]:d}\n')
    ...
    if excflag and nav.ratio < nav.thresar and nav.ratio < 1.5 * nav.prev_ratio2:
        ...
        trace(3, f'AR: restore sat {excsat:d}\n')
...
def detslp_dop(rcv, nav, obs, ix):
    ...
    for i, ii in enumerate(ix):
        ...
        for f in range(nav.nf):
            if (
                obs.L[ii, f] == 0.0
                or obs.D[ii, f] == 0.0
                or nav.ph[rcv, sat, f] == 0.0
                or nav.pt[rcv, sat, f] is None
            ):
                continue
    ...
    if ndop == 0:
        trace(4, f'detslp_dop rcv={rcv + 1:d}: no valid doppler diffs\n')
    ...
    for i, ii in enumerate(ix):
        ...
        for f in range(nav.nf):
            ...
            if abs(dopdif[i, f] - mean_dop) > nav.thresdop:
                ...
                trace(
                    3,
                    f"slip detected doppler (sat={sat + 1:2d} rcv={rcv + 1:d} "
                    f"dL{f + 1:d}={dopdif[i, f] - mean_dop:.3f} "
                    f"off={mean_dop:.3f} tt={tt[i, f]:.2f})\n",
                )
...
def detslp_gf(nav, obsb, obsr, iu, ir):
    ...
    for i in range(ns):
        ...
        if L1R == 0.0 or L1B == 0.0 or L2R == 0 or L2B == 0:
            trace(4, f'gf: skip sat {sat:d}, L=0\n')
            continue
        ...
        if gf0 != 0.0 and abs(gf1 - gf0) > nav.thresslip:
            nav.slip[sat, 0] |= 1
            nav.slip[sat, 1] |= 1
            trace(3, f"slip detected GF jump (sat={sat + 1:2d} L1-L2 dGF={gf0 - gf1:.3f})\n")
...
def detslp_ll(nav, obs, ix, rcv):
    ...
    for f in range(nav.nf):
        ...
        hc_slip = np.where((obs.lli[ix[ixL], f] & 2) != (LLI[ixsat[ixL], f] & 2))[0]
        if len(hc_slip) > 0:
        ...
        if len(slipsats) > 0:
            trace(3, f'slip detected from LLI flags: f={f:d}, sats={slipsats}\n')
            trace(3, f'   slip={slip[ixsat[ixL[ixslip]], f]}\n')


def udpos(nav, sol):
    """states propagation for kalman filter"""
    tt = nav.tt
    trace(3, f'udpos : tt={tt:.3f}\n')
    ...
    if posvar > nav.sig_p0**2:
        ...
        trace(2, f'reset rtk position due to large variance: var={posvar:.3f}\n')
        return
    ...
    if posvar < nav.thresar1:
        ...
        trace(3, f'pos var too high for accel term: {posvar:.4f}\n')
    # x=F*x, P=F*P*F
    nav.x = F @ nav.x
    nav.P = F @ nav.P @ F.T
...
def udbias(nav, obsb, obsr, iu, ir):
    trace(3, f'udbias  : tt={nav.tt:.3f} ns={len(iu):d}\n')
    ...
    for f in range(nav.nf):
        for i in range(uGNSS.MAXSAT):
            ...
            if nav.outc[i, f] > nav.maxout and nav.x[ii] != 0.0:
                trace(
                    3,
                    f'  obs outage counter overflow ( sat={i + 1} L{f + 1}: n={nav.outc[i, f]}\n',
                )
                initx(nav, 0, 0, ii)
            # TODO: set AR minlock
        ...
        for i in range(ns):
            ...
            if (nav.slip[sat[i] - 1, f] & 1) or nav.rejc[sat[i] - 1, f] > 1:
                trace(
                    4,
                    f'flag phase for reset: sat={sat[i]:d} f={f:d} '
                    f'slip={nav.slip[sat[i] - 1, f]:d} rejc={nav.rejc[sat[i] - 1, f]:d}\n',
                )
                initx(nav, 0, 0, j)
        ...
        trace(4, f'phase-code coherency adjust={offset:.2f}, n={namb:d}\n')
        ...
        for i in range(ns):
            ...
            trace(3, f"     sat={sat[i]:3d}, F={f + 1:d}: init phase={bias[i]:.3f}\n")
...
def selsat(nav, obsr, obsb, elb):
    ...
    trace(3, f'selsat  : nu={len(obsr.sat):d} nr={len(obsb.sat):d}\n')
...
def relpos(nav, obsr, obsb, sol):
    ...
    trace(1, "\n---------------------------------------------------------\n")
    trace(1, f"relpos: nu={len(obsr.sat):d} nr={len(obsb.sat):d}\n")
    trace(
        1,
        '        teph= {:04d}/{:02d}/{:02d} {:02d}:{:02d}:{:06.3f}\n'.format(
            ep[0], ep[1], ep[2], ep[3], ep[4], ep[5]
        ),
    )
    trace(1, "---------------------------------------------------------\n")
    if abs(nav.dt) > nav.maxage:
        trace(3, f'Age of differential too large: {nav.dt:.2f}\n')
        return
    ...
    if nav.interp_base:
        ...
        yr0, _, _ = zdres(nav, nav.obsb, nav.rsb, nav.dtsb, nav.svhb, nav.varb, nav.rb, 0)
...
def rtkpos(nav, rov, base, dir):
    """relative positioning for PPK"""
    trace(3, f'rtkpos: start solution, dir={dir:d}\n')
    n = 0
    sol = gn.Sol()
